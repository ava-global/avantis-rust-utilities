...
impl KafkaAgent {
    pub fn new(kafka_config: KafkaConfig) -> KafkaAgent {
        let mut kakfa_agent = KafkaAgent {kafka_config, kafka_future_producer: None};
        kakfa_agent.kafka_future_producer =  Some(kakfa_agent.producer_config::<FutureProducer>().unwrap());
        kakfa_agent
    }
